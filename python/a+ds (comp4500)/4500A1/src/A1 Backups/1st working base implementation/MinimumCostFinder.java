...
public class MinimumCostFinder {
	...
	*/
	 ...
	 */
	 public static int findMinimumCost(
		...
		for (Delivery startDelivery: deliveries){
			...
			if (startDelivery.source().equals(source)){
				...
				if (startDelivery.destination().equals(destination)){
					nextRouteCost = startDelivery.cost();
				}
				else {
						...
						nextRouteCost = findRoute(locations, startDelivery.destination(), 
								startDelivery.arrival(), destination, td, deliveries, startDelivery.cost());
					System.out.println("Route " + nextRouteCost);
				}
					//does this ensure correctness here?
				if (nextRouteCost >= 0 && (nextRouteCost < minCost || minCost == -1)){
					minCost = nextRouteCost;
				}
		...
		//if no adjacent routes, minCost will be unchanged
		return minCost;
	}
	 
	//how do I make sure I'm not traversing through the graph a needless amount of times
		//i.e. ignoring the neighbour routes i didnt choose, or even just looking at the routes
	...
	*/
	private static int findRoute(Set<Location> locations, //why do i need locations?
		...
		int minRouteCost = -1; //we want to return this
		int newRouteCost = -1; //use this as temp to check against current
		...
		for (Delivery nextDelivery: deliveries){
			...
			//so could the time for delivery across the whole route be the same?
			//e.g. 0 0 0 0 0 0 0 0 0 ?
			if (nextDelivery.source().equals(source)
					&& ts <= nextDelivery.departure() 
					&& nextDelivery.departure() <= nextDelivery.arrival()){ 
				
				if (nextDelivery.destination().equals(destination)){
					newRouteCost = currentRouteCost + nextDelivery.cost();
				}
				else {
					//this will return the cheapest possible route from the graph for the given delivery
					newRouteCost = currentRouteCost + nextDelivery.cost();
					newRouteCost = findRoute(locations, nextDelivery.destination(), nextDelivery.arrival(), destination, td, deliveries, newRouteCost);
				}
				
				if (newRouteCost >= 0 && (newRouteCost < minRouteCost || minRouteCost == -1)){
					minRouteCost = newRouteCost;
				}
			}
